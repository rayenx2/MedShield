  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        console.warn('Hospital proof document upload failed (registration still succeeded):', proofErr.message);
      }
    }
...
STRICT OUTPUT FORMAT (each section starts a new line with this exact label):
DOCUMENT IDENTIFIED: [Type]
KEY FINDINGS: [Bullet points]
ALERTS: [Parameter], [Value] vs [Range], [Meaning]
...
RULES:
- Never diagnose.
- Always flag abnormal values under ALERTS.
- Be empathetic.
- Mark lesions, masses, or fractures as high priority under ALERTS.`;
    ...
    try {
      ...
      if (fetchErr.name === 'AbortError' || fetchErr.code === 'UND_ERR_CONNECT_TIMEOUT') {
        console.error('TIMEOUT: Cannot reach Groq API servers. Check network/firewall.');
